...
struct pg_limiter {
	uint64_t next_date;
	uint64_t last_go_date;
	double pkt_size_mean;
	uint16_t burst_size_max;
	uint64_t bps;
};

/* Initilize poll limiter
 *
 * @burst_size_max	packets per burst to target
...
/* Update poll limiter
 *
 * @limiter	limiter structure
 * @pkts	current burst
 * @count	number of packets in burst
 */
void pg_limiter_update(struct pg_limiter *limiter,
